    ...
    <div class="wr-transactions-intro">
      <div class="container">
        <div class="page-navigation">
          ...
          <ul>
            <li>
              <a v-show="!!website" :href="website" target="_blank" rel="nofollow">{{ $t('target.ABOUT_TARGET') }}</a>
            </li>
          </ul>
        </div>
        <div class="transactions-intro">
          <header>
            <p>
              <router-link :to="{name: 'foundation', params: {id:idFoundation}}">{{ foundationName }}</router-link>
            </p>
            <h1>{{ name }}</h1>
          </header>
          <section>
      ...
      <div class="container">
        ...
        <div class="transactions-info">
          <div :class="{'transaction-tab':true,'income':true,'active':tab==1}">
            ...
            <ul>
              <li v-for="(item, index) in donations" :class="{ active: item.isMyDonation && item.isMyDonation == 1 }">
          ...
          <div :class="{'transaction-tab':true,'expense':true,'active':tab==2}">
            ...
            <ul>
              <li v-for="(item, index) in expenses">
                <span class="date" v-if="item.dDoc">{{ item.dDoc | date }}</span>
                <p>{{ item.sum | localeString | rub }}</p>
                <ul>
                  <li v-if="item.costItemName || item.remark">
                    <span>{{ $t('common.EXPENSE_DESCRIPTION') }}</span>
                    <p v-if="item.costItemName">{{ item.costItemName }}</p>
  ...
  export default {
    ...
    methods: {
      fetch () {
        ...
        return foundationApi.find.target.one(this.idFoundation, this.idTarget, filter)
          .then((data) => {
            this.name = data.target.name
            this.foundationName = data.target.nameFoundation
            this.website = data.target.website
